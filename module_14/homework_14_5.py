...
class RegistrationState(StatesGroup):
    """Новый класс регистрации пользователей."""
    username = State()
    email = State()
    age = State()
    balance = State()
...
async def set_username(message, state):
    ...
    if is_included(username):
        ...
        await RegistrationState.username.set()  # Остаемся в состоянии username
    else:
        # Если пользователя нет, сохраняем имя и переходим к следующему состоянию
...
async def set_age(message, state):
    ...
    # Сохраняем возраст и получаем все данные
    await state.update_data(age=age)
    data = await state.get_data()

    # Добавляем пользователя в базу данных
    add_user(username=data['username'], email=data['email'], age=int(data['age']))
...
async def get_buying_list(message):
    ...
    for product in products:
        ...
        image_path = f'foto/{product_id}.png'  # module_14/foto/{product_id}.png (foto/{product_id}.png)
