...
async def price(message):
    await message.answer(text="Что вы хотите купить?", reply_markup=catalog_kb)


@dp.callback_query_handler(text="medium")
async def buy_m(call):
    await call.message.answer(texts.Medium_game, reply_markup=buy_kb)
    await call.answer()


@dp.callback_query_handler(text="big")
async def buy_b(call):
    await call.message.answer(texts.Large_game, reply_markup=buy_kb)
    await call.answer()


@dp.callback_query_handler(text="mega")
async def buy_m(call):
    await call.message.answer(texts.XL_game, reply_markup=buy_kb)
    await call.answer()
